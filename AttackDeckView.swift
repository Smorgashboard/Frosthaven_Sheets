//
//  AttackDeckView.swift
//  Frost_digital_sheets
//
//  Created by Hackin Tosh on 3/9/23.
//

import SwiftUI
import CoreData

struct AttackDeckView: View {
    @EnvironmentObject var settings: Settings
    var characterId: UUID
    @State private var attackDeck = [""]
    @State var currentAttack : String?
    @State private var drawnCards: [String] = []
    @State private var flipped: Bool = false
    @State private var shuffleNeeded: Bool = false
    @State private var shuffleOverride: Bool = false
    @State private var displayShuffleWarn: Bool = false
    @State private var InfuserAlert: Bool = false
    
    var connerCount: Int = 0
    ...
    @State private var curseNumber: Int = 0
    @State private var blessNumber: Int = 0
    @State private var extraNegOne: Int = 0
    ...
    var body: some View {
        ...
        VStack {
            HStack{
                ...
                Menu{
                    ...
                    Button("Add -1") {
                        addAttack("-1")
                        extraNegOne = extraNegOne + 1
                    }
                    Text("Blesses: \(blessNumber)")
                    Text("Curses: \(curseNumber)")
                    Text("Extra -1s: \(extraNegOne)")
                } label: {
            ...
            HStack{
                Button("Draw") {
                    if shuffleOverride == false && !drawnCards.isEmpty && ( drawnCards[0] == "x2" || drawnCards[0] == "x0") {
                        shuffleNeeded = true
                    }
                    else {
                    let attack = getRandomAttack()
                    drawnCards.insert(attack, at: 0)
                    flipped = false
                ...
                .clipShape(Capsule())
                .alert(isPresented: $shuffleNeeded) {
                    Alert(
                        title: Text("Shuffle Needed!"),
                        message: Text("You forgot to shuffle! Would you like to shuffle?" ),
                        primaryButton: .default(Text("Shuffle")) {
                            shuffleNeeded = false
                            currentAttack = shuffleDeck()
                        },
                        secondaryButton: .cancel(){
                            shuffleNeeded = false
                            shuffleOverride = true
                        }
                    )
                }
                Button("Shuffle") {
            ...
            ZStack {
                ...
                Image("\(currentAttack ?? "backcard")")
                    ...
                    .clipShape(Rectangle())
                    .shadow(radius: 5)
                    .rotation3DEffect(.degrees(flipped ? 359.9 : 0), axis: (x: 0, y: 1, z: 0))
        ...
        .onAppear{
            self.attackDeck = buildDeck(playableClass: character.first!.playableClass!)
        }
    }
    
    
    //Start Deck Building Functions
    
    func buildDeck(playableClass: String) -> [String] {
    ...
    func buildSnowDeck() -> [String] {
        ...
        func getRandomAttack() -> String {
            guard !attackDeck.isEmpty else {
                    return "backcard"
                }
            if settings.connerMode {
                
                var connerCount = 0
                let shouldMoveToBack: (String) -> Bool = { element in
                    return element == "-1" || element == "-2" || element == "x0"
                }
                let partitionIndex = attackDeck.partition(by: shouldMoveToBack)
                
                connerCount = attackDeck.count - partitionIndex
                
                attackDeck = Array(attackDeck[..<partitionIndex]) + Array(attackDeck[partitionIndex...])
                
                //This will never fire.
                if connerCount == 0 {
                    let randomIndex = Int.random(in: 0..<attackDeck.count)
                    let attack = attackDeck[randomIndex]
                    if attack == "x2" || attack == "x0" {
                        displayShuffleWarn = true
                    }
                    attackDeck.remove(at: randomIndex)
                    return attack
                }
                let semirandomIndex = Int.random(in: 0..<connerCount)
                let attack = attackDeck[semirandomIndex]
                if attack == "x2" || attack == "x0" {
                    displayShuffleWarn = true
                }
                attackDeck.remove(at: semirandomIndex)
                return attack
            }
            let randomIndex = Int.random(in: 0..<attackDeck.count)
            let attack = attackDeck[randomIndex]
    ...
    func shuffleDeck() -> String {
        ...
        var blesses = 0
        drawnCards = []
        shuffleOverride = false
        shuffleNeeded = false
        displayShuffleWarn = false
        for i in attackDeck{
            if i == "curse"{
                curses = curses + 1
            }
        ...
        attackDeck = buildDeck(playableClass: character.first!.playableClass!)
        if blesses != 0 {
            for _ in 1...blesses {
                attackDeck.append("bless")
            }
        }
        if curses != 0 {
            for _ in 1...curses {
                attackDeck.append("curse")
            }
            attackDeck.shuffle()
        }
        return "backcard"
        ...
        func addAttack(_ attack: String) {
            attackDeck.append(attack)
        }
